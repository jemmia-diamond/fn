...
export default class ManualPaymentService {
  ...
  static async syncManualPaymentsToDatabase(env) {
    ...
    if (!manualPaymentTable) {
      console.error(
        "MANUAL_PAYMENT table configuration not found in constants.",
      );
      return;
    }
        ...
        if (response.code === 0 && response.data) {
          ...
          if (currentRecordsPage.length > 0) {
            const upsertPromises = currentRecordsPage.map((record) => {
              ...
              const data = {
                ...
                transfer_note: Array.isArray(fields["Nội Dung CK"])
                  ...
                  : fields["Nội Dung CK"],
                haravan_order_id:
                  fields["ORDER ID"]?.value?.[0]?.toString() ?? null,
            ...
            try {
              ...
              console.error(
                "Error during bulk upsert of manual payments for current page:",
                error,
              );
            }
          }
