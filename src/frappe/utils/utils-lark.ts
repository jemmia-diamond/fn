...
import { IFrappeLead, IFrappeSalesPerson, ILarksuiteAppointment, LarksuiteSalePerson } from "services/larksuite/appointment/types";
...
export async function getAllSalesPersons(frappeClient: FrappeClient, employeeEmails: string[], names: string[]): Promise<IFrappeSalesPerson[]> {
...
export async function mapLarkToFrappe(frappeClient: FrappeClient, dataRequest: ILarksuiteAppointment, lead: IFrappeLead | null) {
    ...
    const salesPersons = (allEmails.length > 0 || allNames.length > 0) ? await getAllSalesPersons(frappeClient, allEmails, allNames) : [];
