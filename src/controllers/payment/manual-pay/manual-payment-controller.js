...
export default class ManualPaymentsController {
  ...
  static _parseAndTypecast(body) {
    ...
    ["send_date", "receive_date", "created_date", "updated_date"].forEach(
      (field) => {
  ...
  static async create(c) {
    try {
      ...
      const newPayment =
        await manualPaymentService.createManualPayment(paymentData);

      if (newPayment) {
        return c.json(
          ManualPaymentsController._serializePayment(newPayment),
          201
        );
      } else {
  ...
  static async update(c) {
    try {
      ...
      if (updatedPayment) {
        return c.json(
          ManualPaymentsController._serializePayment(updatedPayment),
          200
        );
      } else {
