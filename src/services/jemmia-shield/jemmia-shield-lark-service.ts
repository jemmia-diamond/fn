...
export default class JemmiaShieldLarkService {
  ...
  static async getUserGroups(
    ...
    try {
      ...
      while (hasMore) {
        const response: any = await JemmiaShieldLarkService.userClient(
          userAccessToken
        ).get("/im/v1/chats", {
          params: {
  ...
  static async addBotAsManager(
    ...
    try {
      const LARK_APP_ID = env.LARK_APP_SHIELD_ID;
      const response = await JemmiaShieldLarkService.userClient(
        userAccessToken
      ).post(
  ...
  static async uploadImage(env: any, imageBuffer: Buffer): Promise<string> {
    try {
      const appAccessToken =
        await JemmiaShieldLarkService.getAppAccessToken(env);
      ...
      const response = await fetch(
        `${JemmiaShieldLarkService.API_BASE}/im/v1/images`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${appAccessToken}`
