...
export default class LeadService {
  ...
  async processCallLogLead(data) {
    ...
    const source = await this.getLeadSource("CallLog");
    if (!source) {
      return;
    }
    ...
    const lead = await this.getOrCreateLead(phone, dataBuilder);
    if (lead) {
      await contactService.processCallLogContact(data, lead, source);
    }
  }

  static async syncCallLogLead(env) {
    const leadService = new LeadService(env);
    const timeThreshold = dayjs().utc().subtract(10, "minutes").format("YYYY-MM-DD HH:mm:ss");
