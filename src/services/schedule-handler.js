...
export default {
  scheduled: async (controller, env, _ctx) => {
    ...
    case "30 1 * * *": // 08:30
      ...
      await new Payment.MisaVoucherCreator(env).runMorningBatch();
      break;
    case "30 5 * * *": // 12:30
      await ERP.Automation.AssignmentRuleService.updateAssignmentRulesMidDay(env);
      break;
    case "30 6 * * *": // 13:30
      await new Payment.MisaVoucherCreator(env).runAfternoonBatch();
      break;
    case "0 10 * * *": // 17:00
      await ERP.Automation.AssignmentRuleService.updateAssignmentRulesEndDay(env);
    case "0 11 * * *": // 18:00
      await new Payment.MisaVoucherCreator(env).runEndOfDayBatch();
      break;
    case "0 14 * * *": // 21:00
